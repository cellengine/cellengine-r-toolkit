...
test_that("makes expected HTTP request", {
  with_mock(
    `httr::request_perform` = function(req, handle, refresh) {
      ...
      expect_equal(req$url, "https://my.server.com/api/v1/experiments/591a3b441d725115208a6fda/fcsfiles/591a3b441d725115208a6fdc.FCS?compensatedQ=FALSE&headers=TRUE")
...
test_that("makes expected HTTP request with subsampling", {
  with_mock(
    `httr::request_perform` = function(req, handle, refresh) {
      expect_equal(req$method, "GET")
      expect_equal(req$url, "https://my.server.com/api/v1/experiments/591a3b441d725115208a6fda/fcsfiles/591a3b441d725115208a6fdc.FCS?compensatedQ=FALSE&headers=TRUE&preSubsampleN=50&seed=2.25")
      response = httptest::fake_response(
        req$url,
        req$method,
        content=readBin("../5k.fcs", raw(), file.info("../5k.fcs")$size),
...
test_that("doesn't mangle column names", {
  with_mock(
    `httr::request_perform` = function(req, handle, refresh) {
      expect_equal(req$method, "GET")
      expect_equal(req$url, "https://my.server.com/api/v1/experiments/591a3b441d725115208a6fda/fcsfiles/591a3b441d725115208a6fdc.TSV?compensatedQ=FALSE&headers=TRUE")
      response = httptest::fake_response(
        req$url,
        req$method,
